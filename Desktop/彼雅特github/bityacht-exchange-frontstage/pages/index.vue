<template>
  ...
  <section>
    <div v-if="bannerReady" class="relative">
      <a-carousel ref="carouselRef" effect="fade" autoplay>
        <div v-for="item of bannerList" :key="item.webImage" :class="`relative md:-mt-[40px] mt-[72px]`">
          <img :src="isMobile ? item.appImage : item.webImage" class="w-full" />
          <div class="absolute 3xl:top-[30%] md:top-[25%] top-[20%] 3xl:left-[18%] md:left-[12%] left-[8%]">
            <p class="3xl:w-[550px] xl:w-[450px] md:w-[400px] w-[90%] 2xl:text-h2 lg:text-h5 md:text-[26px] text-[24px] text-white">{{ item.title }}</p>
            <p class="3xl:w-[300px] md:w-[250px] w-[85%] md:text-subTitle text-subText text-white mt-2 font-light">{{ item.subTitle }}</p>
            <NuxtLink :to="item.buttonUrl" target="_blank">
              <Button variant="waterBlue" class="mt-6">{{ item.buttonText }}</Button>
            </NuxtLink>
          </div>
        </div>
      </a-carousel>
  ...
  <section class="2xl:px-[300px] xl:px-[200px] lg:px-[100px] px-[50px]">
    ...
    <div class="flex justify-center flex-wrap md:mt-[62px] mt-[30px]">
      <div v-for="item of whyChoice" class="3xl:w-[315px] md:w-[48%] w-[95%] border-2 border-waterBlue rounded-xl p-2 mr-2 mb-2">
        <div class="flex flex-col items-center border border-waterBlue rounded-xl p-3">
          <div class="2xl:w-[200px] xl:w-[150px] w-[120px] xl:h-[150px] h-[110px] xl:mb-6 mb-2">
  ...
  <section class="2xl:px-[300px] xl:px-[200px] lg:px-[100px] px-[50px]">
    <div class="flex items-center mt-[45px] mb-[75px] md:justify-start justify-center">
      <span class="block h-[44px] border border-waterBlue"></span>
      <h1 class="md:text-h2 text-h5 text-white pl-6">{{ $t('index.marketTrend') }}</h1>
    </div>
    <div>
      <div class="flex items-center justify-center">
        <p class="text-white md:text-h3 text-[28px] w-[50%] text-center cursor-pointer" :class="`${isBuy ? `border-t border-l border-r border-waterBlue rounded-tr-lg` : `border-b border-waterBlue`}`" @click="marketAction('buy')">{{ $t('index.buyCurrency') }}</p>
        <p class="text-white md:text-h3 text-[28px] w-[50%] text-center cursor-pointer" :class="`${isBuy ? `border-b border-waterBlue` : `border-t border-l border-r border-waterBlue rounded-tl-lg`}`" @click="marketAction('sale')">{{ $t('index.saleCurrency') }}</p>
      </div>
      <div class="border-l border-r border-b border-waterBlue">
        <template v-if="!isMobile">
          <Table :columns="columns" :data="tableData" :pagination="false" class="indexTable">
            <template v-slot:symbol="{ record }">
              <template v-if="record.symbol === 'BTC'">
                <div class="flex items-center">
                  <img src="/assets/img/BTC.svg" class="w-10 h-10 mr-3" />
                  <div class="flex flex-col items-start">
                    <p class="font-bold text-subTitle text-white">{{ record.symbol }}</p>
            ...
            <template v-slot:updown="{ text }">
              <span v-if="Number(text) > 0" class="text-orange">{{ text }}%</span>
              <span v-else class="text-green">{{ text }}%</span>
            </template>
            <template v-slot:trend="{ record }">
              <Echarts :id="record.symbol" :series="record.trend" :max="record.max" :min="record.min" location="index" :show="false" symbol="none" :inverse="false" />
              <!-- <p>{{ record }}</p> -->
            </template>
          </Table>
        </template>
        <template v-else>
          <div class="flex p-5" v-for="(item, index) of tableData" :key="`${item.symbol} + ${index}`">
            <div class="w-[50%] flex items-center">
              <img :src="getAssetsFile(`${item.symbol}.svg`)" class="xxs:w-[50px] w-[30px] xxs:h-[50px] h-[30px]" />
              <div class="flex flex-col items-start xxs:ml-4 ml-1">
                <p class="font-bold text-subTitle text-white">{{ item.symbol }}</p>
                <p class="text-subText text-grayText">{{ item.name }}</p>
              </div>
            </div>
            <div class="w-[50%] flex flex-col items-end">
              <p v-if="isBuy" class="font-bold xxs:text-subTitle text-subText text-white">≈ $ {{ formatValue(item.buyPrice) }}</p>
              <p v-else class="font-bold xxs:text-subTitle text-subText text-white">≈ $ {{ formatValue(item.sellPrice) }}</p>
              <p v-if="Number(item.upsAndDowns) > 0" class="xxs:text-subText text-small text-orange">{{ item.upsAndDowns }} %</p>
              <p v-else class="xxs:text-subText text-small text-green">{{ item.upsAndDowns }} %</p>
            </div>
          </div>
        </template>
      </div>
  ...
  <section class="relative 2xl:px-[300px] xl:px-[200px] lg:px-[100px] px-[50px]">
    <div class="flex items-center mt-[85px] mb-[75px] md:justify-start justify-center">
      <span class="block h-[44px] border border-waterBlue"></span>
      <h1 class="md:text-h2 text-h5 text-white pl-6">{{ isMobile ? $t('index.signUpIsEasy') : $t('index.whySignUpEasy') }}</h1>
    </div>
    <div class="flex items-center justify-between">
      ...
      <div class="md:w-[50%] w-full">
        <div v-for="item of signUpList" class="flex mb-4 ml-10">
          <img :src="getAssetsFile(item.img)" class="xxs:w-[115px] w-[100px]" />
          <div class="flex flex-col md:ml-10 ml-4 mt-5">
            <div class="flex items-end">
              <p class="text-waterBlue xl:text-h2 lg:text-h4 md:text-[28px] text-[24px]">{{ $t(`${item.title}`) }}</p>
              <span class="block text-white xl:text-h5 md:text-[25px] text-[20px] xl:ml-[30px] lg:ml-[20px] ml-[10px] align-bottom">{{ item.type }}</span>
            </div>
            <div class="text-white xl:text-subTitle md:text-normal text-small">{{ $t(`${item.subTitle}`) }}</div>
            <div v-if="item.subTitle2" class="text-white xl:text-subTitle md:text-normal text-small">{{ $t(`${item.subTitle2}`) }}</div>
          </div>
        </div>
      </div>
    </div>
    <img src="assets/img/Group16.png" class="absolute top-0 right-0 md:w-[700px] w-[350px]" />
    <img src="assets/img/Group37.png" class="absolute left-0 top-90 md:w-[550px] w-0" />
  </section>
  <section>
    <div class="relative bg-grayBg md:mt-[120px] mt-[60px] z-20">
      <div class="2xl:px-[300px] xl:px-[200px] lg:px-[100px] px-[50px] py-10 flex justify-center">
        <div class="md:flex hidden">
          <img src="assets/img/takeHands.png" class="w-[50%]" />
          <div class="w-[50%] flex flex-col items-center justify-center">
            <p class="3xl:text-h1 lg:text-h3 text-h5 text-primary">{{ $t('index.waitingForWhat') }}</p>
            <NuxtLink :to="localePath('/signUp')">
              <Button variant="waterBlue" class="xl:w-[290px] w-[200px] xl:text-h4 text-[24px] rounded-4xl mt-8">{{ $t('index.startSignUp') }}</Button>
            </NuxtLink>
          </div>
        </div>
        <div class="md:hidden flex flex-col items-center">
          <p class="xxs:text-h5 text-[26px] font-bold text-primary">{{ $t('index.waitingForWhat') }}</p>
          <img src="assets/img/takeHands.png" class="w-full mb-10" />
          <NuxtLink :to="localePath('/signUp')" class="flex justify-center">
            <Button variant="waterBlue" class="w-[200px] text-[16px] rounded-4xl absolute -bottom-[20px]">{{ $t('index.startSignUp') }}</Button>
          </NuxtLink>
        </div>
      </div>
    </div>
...
.indexTable :where(.css-dev-only-do-not-override-eq3tly).ant-table-wrapper .ant-table-tbody > tr.ant-table-row:hover > td,
.indexTable :where(.css-dev-only-do-not-override-eq3tly).ant-table-wrapper .ant-table-tbody > tr > td.ant-table-cell-row-hover {
